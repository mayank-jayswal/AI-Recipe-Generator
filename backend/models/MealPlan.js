...
class MealPlan {
    ...
    static normalizeMealTypeForDb(mealType) {
        return MEAL_TYPE_TO_DB[mealType?.toLowerCase()] || mealType;
    }
    ...
    */
    static async create(userId, mealData) {
        ...
        const dbMealType = this.normalizeMealTypeForDb(meal_type);

        const result = await db.query(
        `INSERT INTO meal_plans (user_id, recipe_id, meal_date, meal_type)
        VALUES ($1, $2, $3::date, $4)
        ON CONFLICT (user_id, meal_date, meal_type)
        DO UPDATE SET recipe_id = $2, updated_at = CURRENT_TIMESTAMP
        RETURNING *`,
        [userId, recipe_id, date, dbMealType]
        );
        ...
        const enriched = await db.query(
            `SELECT mp.id, mp.user_id, mp.recipe_id, mp.meal_date::text as meal_date,
                    LOWER(mp.meal_type) as meal_type, mp.created_at, mp.updated_at,
